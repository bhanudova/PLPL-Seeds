import { footerLinks } from "@/constraints/global";
import Image from "next/image";

export default function Footer() {
  return (
    <>
      <div className="h-full bg-base-content text-primary-content" >
        <div>
          <div className="flex flex-col lg:flex-row pt-6 px-10 gap-8 lg:gap-4">
              ...
              <div>
                ...
                <div className="flex justify-center md:justify-center lg:justify-normal mt-2 md:mt-0">
                  <div className="flex mt-0 md:mt-3 lg:mt-3">
                    <input type="text" className="outline-none bg-[#666E64] md:bg-[#666E64] lg:bg-[#666E64] rounded-full w-[315px] p-2 text-white font-bold text-sm" placeholder="Your Email" />
                    <button className="m-1 p-2 rounded-full bg-white text-gray-800 font-medium ms-[-88px] text-sm">Subscribe</button>
                  </div>
                </div>
              </div>
            </div>
            <div className="w-full lg:w-[66%] flex justify-center ">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 ml-0 md:ml-20 lg:ml-0 gap-4 lg:gap-10">
                {
                  footerLinks && footerLinks.map(({ name, link, sub }: any, i: number) => (
                    <div key={`footer_links_${i}`}>
                      <a href={link} className="font-bold text-base md:text-lg hover:underline">{name}</a>
                      <ul className="font-medium text-sm md:text-sm">
                        {sub && sub.map(({ name, link }: any) => {
                          <li className="my-1"><a href={link} className="hover:underline hover:text-green-400">{name}</a></li>
                        })}
                      </ul>
                    </div>
                  ))
